...
| <a name="input_ci_config_path"></a> [ci\_config\_path](#input\_ci\_config\_path) | Custom Path to CI config file. | `string` | `".ci-gitlab.yml"` | no |
| <a name="input_ci_default_git_depth "></a> [ci\_default\_git\_depth ](#input\_ci\_default\_git\_depth ) | Default number of revisions for shallow cloning. | `number` | `1` | no |
...
| <a name="input_pipeline_schedule"></a> [pipeline\_schedule](#input\_pipeline\_schedule) | n/a | <pre>object({<br>    cron           = string<br>    activate       = optional(bool)<br>    cron_timezone  = optional(string)<br>    description    = optional(string)<br>    take_ownership = optional(bool)<br>    ref            = optional(string)<br>  })</pre> | <pre>{<br>  "activate": true,<br>  "cron": null,<br>  "cron_timezone": "UTC",<br>  "description": null,<br>  "ref": "main",<br>  "take_ownership": false<br>}</pre> | no |
